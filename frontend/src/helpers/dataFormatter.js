...
export default {
    ...
    dataGridEditFormatter(obj) {
        ...
        faxesManyListFormatter(val) {
            if (!val || !val.length) return []
            return val.map((item) => item.sender)
        },
        faxesOneListFormatter(val) {
            if (!val) return ''
            return val.sender
        },
        faxesManyListFormatterEdit(val) {
            if (!val || !val.length) return []
            return val.map((item) => {
              return {id: item.id, label: item.sender}
            });
        },
        faxesOneListFormatterEdit(val) {
            if (!val) return ''
            return {label: val.sender, id: val.id}
        },
